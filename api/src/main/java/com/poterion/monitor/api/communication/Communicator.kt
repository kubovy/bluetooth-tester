		...
		try {
			while (!Thread.interrupted() && state == State.CONNECTED) try {
				if (checksumQueue.isNotEmpty()) {
					...
					sendMessage(data)
					LOGGER.debug("${channel} ${connectionDescriptor}> Outbound CRC: ${"0x%02X".format(chksum)} (${checksumQueue.size})")
					...
					if (iddCounter < 0) {
						...
						val message = if (iddState < 2) arrayOf(MessageKind.IDD.code, Random.nextBits(4), iddState) else
							arrayOf(MessageKind.IDD.code, Random.nextBits(4))
						messageQueue.add(message.map { it.toByte() }.toByteArray() to 500)
					}
				}
			} catch (e: Exception) {
				LOGGER.error("${channel} ${connectionDescriptor}> ${e.message}")
